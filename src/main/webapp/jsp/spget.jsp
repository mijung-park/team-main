...
<script>
var appRoot = '${appRoot}';
var seq = ${board.cs_seq};
var nickname = '${authUser.member_nickname}';
var grade = '${authUser.member_grade}';
var img = '${getCsfileNameList }';
</script>
<script
...
<script src="${appRoot }/resources/js/cs_reply.js"></script>
...
//	reply_filename VARCHAR2(3000)
	...
	// 댓글 목록 가져오기
	function showList() {			
		replyService.getList({cs_seq: seq}, function(list) {
...
<body>
	<section id="container">
		<div class="webzineTypeView">
			<div class="headWrap">
				<p class="mr-t10">${board.cs_title }</p>
				<div class="date">
					<p><strong>구분 : </strong>${board.cs_category }</p>
					<p><strong>조회수 : </strong>${board.cs_readcnt }</p>
					<p>
						<strong>작성자 : ${board.cs_writer } </strong>
					</p>
					<c:choose>
						<c:when test="${empty board.cs_updatedate}">
							<p>
								<strong>등록일 :</strong>
								<fmt:formatDate value='${board.cs_regdateKST}' pattern='yyyy년 MM월 dd일 hh시 mm분'/>
							</p>
						</c:when>
						<c:otherwise>
							<p>
								<strong>수정일 :</strong>
								<fmt:formatDate value='${board.cs_updatedateKST}' pattern='yyyy년 MM월 dd일 hh시 mm분'/>
							</p>
						</c:otherwise>
					</c:choose>
				</div>
			...
			<div class="bodyWrap">
				<div class="form-group" style="margin-top:5px;">
					<c:forEach items="${getCsfileNameList }" var="csImg" varStatus="imgNum">
						<img onerror="this.src='${appRoot }/resources/noimage.jpg'" src="${appRoot }/resources/cs/upload/${csImg }" width="245px" height="245px">
					</c:forEach>
				</div>
				<p>${board.cs_content }</p>
			</div>
		</div>
		<c:url value="/cs/list" var="listLink">
			<c:param name="cs_seq" value="${board.cs_seq }" />
		...
		<c:if test="${board.cs_writer == authUser.member_nickname && authUser.member_grade == 1 }">
			<a id="btn_add" style="margin-right: 3px;" href="${appRoot }/cs/modify?cs_seq=${board.cs_seq }" >수정</a> 
			<a id="btn_add" style="margin-right: 3px;" href="${appRoot }/cs/remove?cs_seq=${board.cs_seq }" >삭제</a>				
		</c:if>
		<!-- 등급이 0 , 곧 관리자 일경우만 모든 게시물 삭제 가능 하도록 활성화 -->
		<c:if test="${authUser.member_grade == 0 }">
			<a id="btn_add" style="margin-right: 3px;" href="${appRoot }/cs/remove?cs_seq=${board.cs_seq }">삭제</a>
		</c:if>		
 		<!-- 관리자이면서 자신의 글일 경우  -->
 		<c:if test="${authUser.member_grade == 0 && board.cs_writer == authUser.member_nickname }">
			<a id="btn_add" style="margin-right: 3px;" href="${appRoot }/cs/modify?cs_seq=${board.cs_seq }">수정</a> 
		</c:if>
	<!-- 댓글 작성 폼 -->
	<div class="recommWritebox">
		<div class="lws">
			...
			<c:if test="${not empty authUser.member_nickname}">
			<!-- // 작성자 hidden 처리 -->	
				<input readonly type="hidden" type="text" name="reply_writer" value="${authUser.member_nickname }" id="reply_writer_input"/><br>
				<c:if test="${errors.reply_noContent }">
					<small class="form-text text-danger"> 댓글 내용을 입력 해주세요. </small>
				</c:if>
		
			<!-- 일반 유저 댓글 쓰기 -->
				<c:if test="${!empty authUser.member_id && authUser.member_grade == 1}">
					<button class="replyBtn_add" id="reply-submit-button">댓글작성</button> 
				</c:if>
			<!-- 관리자 댓글 쓰기 -->
				<c:if test="${!empty authUser.member_id && authUser.member_grade == 0}">
